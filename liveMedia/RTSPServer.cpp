...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort) {
  ...
  if (fHTTPServerSocket >= 0) {
    ...
    envir().taskScheduler()
        .turnOnBackgroundReadHandling(fHTTPServerSocket,
...
RTSPServer::~RTSPServer() {
  ...
  while ((r = (RTSPRegisterOrDeregisterSender *)
      fPendingRegisterOrDeregisterRequests->getFirst()) != NULL) {
    delete r;
  }
...
void RTSPServer
::noteTCPStreamingOnSocket(int socketNum,
                           RTSPClientSession *clientSession,
                           unsigned trackNum) {
  streamingOverTCPRecord *sotcpCur = (streamingOverTCPRecord *)
      fTCPStreamingDatabase->Lookup((char const *) socketNum);
...
void RTSPServer
::unnoteTCPStreamingOnSocket(int socketNum,
                             RTSPClientSession *clientSession,
                             unsigned trackNum) {
  if (socketNum < 0) return;
  streamingOverTCPRecord *sotcpHead = (streamingOverTCPRecord *)
      fTCPStreamingDatabase->Lookup((char const *) socketNum);
...
void RTSPServer::stopTCPStreamingOnSocket(int socketNum) {
  ...
  streamingOverTCPRecord *sotcp = (streamingOverTCPRecord *)
      fTCPStreamingDatabase->Lookup((char const *) socketNum);
  if (sotcp != NULL) {
    do {
      RTSPClientSession *clientSession = (RTSPServer::RTSPClientSession *)
          lookupClientSession(sotcp->fSessionId);
...
void RTSPServer::RTSPClientConnection
::handleCmd_DESCRIBE(char const *urlPreSuffix,
                     char const *urlSuffix,
...
Boolean RTSPServer::RTSPClientConnection::parseHTTPRequestString(
  ...
  lookForHeader("x-sessioncookie",
                &reqStr[i],
                reqStrSize - i,
                sessionCookie,
                sessionCookieMaxSize);
  lookForHeader("Accept",
                &reqStr[i],
                reqStrSize - i,
                acceptStr,
                acceptStrMaxSize);
...
::handleHTTPCmd_TunnelingPOST(char const *sessionCookie,
  ...
  RTSPServer::RTSPClientConnection *prevClientConnection =
      (RTSPServer::RTSPClientConnection *) (fOurRTSPServer
          .fClientConnectionsForHTTPTunneling->Lookup(sessionCookie));
  ...
  prevClientConnection->changeClientInputSocket(fClientInputSocket,
                                                extraData,
                                                extraDataSize);
...
void RTSPServer::RTSPClientConnection::handleAlternativeRequestByte1(u_int8_t requestByte) {
  if (requestByte == 0xFF) {
    ...
    envir().taskScheduler()
        .setBackgroundHandling(fClientInputSocket,
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
  ...
  do {
    ...
    if (fClientOutputSocket != fClientInputSocket && numBytesRemaining == 0) {
      ...
      // 判定为RTSP-over-HTTP tunneling，其中内容可能使用Base64编码，所以此处
      // 尽可能使用Base64解码
      ...
      if (numBytesToDecode > 0) {
        ...
        newBytesRead =
            decodedSize - fBase64RemainderCount + newBase64RemainderCount;
    ...
    if (parseSucceeded) {
      ...
      if (requestIncludedSessionId) {
        clientSession = (RTSPServer::RTSPClientSession *)
            (fOurRTSPServer.lookupClientSession(sessionIdStr));
      ...
      if (strcmp(cmdName, "OPTIONS") == 0) {
        ...
        handleCmd_DESCRIBE(urlPreSuffix,
                           urlSuffix,
        ...
        if (clientSession != NULL) {
          clientSession->handleCmd_SETUP(this,
                                         urlPreSuffix,
                                         urlSuffix,
                                         (char const *) fRequestBuffer);
        ...
        if (clientSession != NULL) {
          clientSession->handleCmd_withinSession(this,
                                                 cmdName,
                                                 urlPreSuffix,
                                                 urlSuffix,
                                                 (char const *) fRequestBuffer);
      ...
      *fLastCRLF = '\r';
      if (parseSucceeded) {
        ...
        if (strcmp(cmdName, "OPTIONS") == 0) {
          ...
          unsigned extraDataSize =
              &fRequestBuffer[fRequestBytesAlreadySeen] - extraData;
          if (handleHTTPCmd_TunnelingPOST(sessionCookie,
                                          extraData,
                                          extraDataSize)) {
    ...
    send(fClientOutputSocket,
         (char const *) fResponseBuffer,
         strlen((char *) fResponseBuffer),
         0);
...
Boolean RTSPServer::RTSPClientConnection
::authenticationOK(char const *cmdName,
                   char const *urlSuffix,
                   char const *fullRequestStr) {
  ...
  UserAuthenticationDatabase
      *authDB = fOurRTSPServer.getAuthenticationDatabaseForCommand(cmdName);
...
void RTSPServer::RTSPClientSession::deleteStreamByTrack(unsigned trackNum) {
  ...
  if (fStreamStates[trackNum].subsession != NULL) {
    fStreamStates[trackNum]
        .subsession->deleteStream(fOurSessionId,
...
void RTSPServer::RTSPClientSession::reclaimStreamStates() {
  for (unsigned i = 0; i < fNumStreamStates; ++i) {
    if (fStreamStates[i].subsession != NULL) {
      fOurRTSPServer
          .unnoteTCPStreamingOnSocket(fStreamStates[i].tcpSocketNum, this, i);
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection *ourClientConnection,
  ...
  do {
    ...
    if (sms == NULL) {
      ...
      if (urlPreSuffix[0] == '\0') {
        ...
        concatenatedStreamName =
            new char[strlen(urlPreSuffix) + strlen(urlSuffix) + 2]; // allow for the "/" and the trailing '\0'
    ...
    if (token != NULL) {
      ...
      fOurRTSPServer.unnoteTCPStreamingOnSocket(fStreamStates[trackNum].tcpSocketNum,
                                                this,
                                                trackNum);
    ...
    if (parseRangeHeader(fullRequestStr,
                         rangeStart,
                         rangeEnd,
                         absStart,
                         absEnd,
    ...
    if (streamingMode == RTP_TCP) {
      ...
      fOurRTSPServer.noteTCPStreamingOnSocket(fStreamStates[trackNum].tcpSocketNum,
                                              this,
                                              trackNum);
    }
    ...
    if (fOurRTSPServer.fReclamationSeconds > 0) {
      sprintf(timeoutParameterString,
              ";timeout=%u",
...
::handleCmd_withinSession(RTSPServer::RTSPClientConnection *ourClientConnection,
  ...
  if (fOurServerMediaSession == NULL) { // There wasn't a previous SETUP!
    ...
    if (strncmp(fOurServerMediaSession->streamName(),
                urlPreSuffix,
                urlPreSuffixLen) == 0
...
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection *ourClientConnection,
  ...
  for (i = 0; i < fNumStreamStates; ++i) {
    if (subsession == NULL /* means: aggregated operation */
      ...
      if (fStreamStates[i].subsession != NULL) {
        fOurRTSPServer.unnoteTCPStreamingOnSocket(fStreamStates[i].tcpSocketNum,
                                                  this,
                                                  i);
        fStreamStates[i].subsession->deleteStream(fOurSessionId,
                                                  fStreamStates[i].streamToken);
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection *ourClientConnection,
  ...
  if (sawRangeHeader && absStart == NULL/*not seeking by 'absolute' time*/) {
    ...
    duration = subsession == NULL /*aggregate op*/? fOurServerMediaSession->duration() : subsession->duration();
    if (duration < 0.0) {
      ...
      if (fStreamStates[i].subsession != NULL) {
        ...
        if (absStart != NULL) {
          ...
          if (!sawRangeHeader || startTimeIsNow) {
            ...
            fStreamStates[i]
                .subsession->nullSeekStream(fOurSessionId,
                                            fStreamStates[i].streamToken,
                                            rangeEnd,
                                            numBytes);
          } else {
    ...
    if (!sawRangeHeader || startTimeIsNow) {
      ...
      for (i = 0; i < fNumStreamStates; ++i) {
        if (subsession == NULL /* means: aggregated operation */
          ...
          float npt = fStreamStates[i]
              .subsession->getCurrentNPT(fStreamStates[i].streamToken);
...
::handleCmd_GET_PARAMETER(RTSPServer::RTSPClientConnection *ourClientConnection,
  ...
  setRTSPResponse(ourClientConnection,
                  "200 OK",
                  fOurSessionId,
