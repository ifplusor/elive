...
::createRTCP(Groupsock *RTCPgs, unsigned totSessionBW, /* in kbps */
  ...
  // Default implementation; may be redefined by subclasses:
  return RTCPInstance::createNew(envir(),
                                 RTCPgs,
                                 totSessionBW,
                                 cname,
                                 sink,
...
::sendRTCPAppPacket(u_int8_t subtype, char const *name,
  ...
  if (streamState != NULL) {
    streamState->sendRTCPAppPacket(subtype,
                                   name,
                                   appDependentData,
...
::startPlaying(Destinations *dests, unsigned clientSessionId,
  ...
  if (fRTCPInstance == NULL && fRTPSink != NULL) {
    ...
    fRTCPInstance = fMaster.createRTCP(fRTCPgs,
                                       fTotalBW,
                                       (unsigned char *) fMaster.fCNAME,
                                       fRTPSink);
  ...
  if (dests->isTCP) {
    ...
    if (fRTPSink != NULL) {
      ...
      RTPInterface
      ::setServerRequestAlternativeByteHandler(fRTPSink->envir(),
                                               dests->tcpSocketNum,
                                               serverRequestAlternativeByteHandler,
                                               serverRequestAlternativeByteHandlerClientData);
    ...
    if (fRTPgs != NULL)
      fRTPgs->addDestination(dests->addr,
                             dests->rtpPort,
                             clientSessionId);
...
void StreamState::sendRTCPAppPacket(u_int8_t subtype,
  ...
  if (fRTCPInstance != NULL) {
    fRTCPInstance->sendAppPacket(subtype,
                                 name,
                                 appDependentData,
                                 appDependentDataSize);
  }
