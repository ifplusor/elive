...
int main(int argc, char **argv) {
    ...
    switch (opt[1]) {
      ...
      case 'I': { // specify input interface...
        ...
        ReceivingInterfaceAddr =
            *(unsigned *) (addresses.firstAddress()->data());
        ++argv;
        --argc;
        break;
      }
      ...
      case 'P': { // specify an interval (in seconds) between writing successive output files
        ...
        if (sscanf(argv[2], "%d", &fileOutputIntervalInt) != 1
            || fileOutputIntervalInt <= 0) {
          usage();
        }
      ...
      case 'k': { // specify a username and password to be used to authentication an incoming "REGISTER" command (for use with -R)
        ...
        authDBForREGISTER->addUserRecord(usernameForREGISTER,
                                         passwordForREGISTER);
        break;
      }
      ...
      case 'R': {
        ...
        if (argc > 2 && argv[2][0] != '-') {
          ...
          if (sscanf(argv[2], "%hu", &handlerServerForREGISTERCommandPortNum)
  ...
  if (!createReceivers
      && (outputCompositeFile || oneFilePerFrame || fileOutputInterval > 0)) {
  ...
  if (outputCompositeFile && !movieWidthOptionSet) {
    *env
        << "Warning: The -q, -4 or -i option was used, but not -w.  Assuming a video width of "
        << movieWidth << " pixels\n";
  }
  if (outputCompositeFile && !movieHeightOptionSet) {
    *env
        << "Warning: The -q, -4 or -i option was used, but not -h.  Assuming a video height of "
        << movieHeight << " pixels\n";
  }
  if (outputCompositeFile && !movieFPSOptionSet) {
    *env
        << "Warning: The -q, -4 or -i option was used, but not -f.  Assuming a video frame rate of "
  ...
  if (authDBForREGISTER != NULL && !createHandlerServerForREGISTERCommand) {
    *env
        << "If \"-k <username> <password>\" is used, then -R (or \"-R <port-num>\") must also be used!\n";
    usage();
  }
  ...
  if (!createReceivers && notifyOnPacketArrival) {
    *env
        << "Warning: Because we're not receiving stream data, the -n flag has no effect\n";
  }
  if (durationSlop < 0) {
  ...
  if (createHandlerServerForREGISTERCommand) {
    handlerServerForREGISTERCommand
        = HandlerServerForREGISTERCommand::createNew(*env,
                                                     continueAfterClientCreation0,
                                                     handlerServerForREGISTERCommandPortNum,
                                                     authDBForREGISTER,
                                                     verbosityLevel,
                                                     progName);
    if (handlerServerForREGISTERCommand == NULL) {
      *env
          << "Failed to create a server for handling incoming \"REGISTER\" commands: "
          << env->getResultMsg() << "\n";
    } else {
    ...
    if (ourClient == NULL) {
      *env << "Failed to create " << clientProtocolName << " client: "
           << env->getResultMsg() << "\n";
      shutdown();
    }
        ...
        if (subsession->rtpSource() != NULL) {
          ...
          if (socketInputBufferSize > 0 || fileSinkBufferSize > curBufferSize) {
            unsigned newBufferSize =
                socketInputBufferSize > 0 ? socketInputBufferSize
                                          : fileSinkBufferSize;
...
void createOutputFiles(char const *periodicFilenameSuffix) {
  ...
  if (outputQuickTimeFile || outputAVIFile) {
    if (periodicFilenameSuffix[0] == '\0') {
      ...
      char const
          *prefix = fileNamePrefix[0] == '\0' ? "output" : fileNamePrefix;
    ...
    while ((subsession = iter.next()) != NULL) {
      ...
      if (strcmp(subsession->mediumName(), "video") == 0) {
        if (strcmp(subsession->codecName(), "H264") == 0) {
          ...
          fileSink = H264VideoFileSink::createNew(*env,
                                                  outFileName,
          ...
          fileSink = H265VideoFileSink::createNew(*env,
                                                  outFileName,
        ...
        if (strcmp(subsession->codecName(), "AMR") == 0 ||
          ...
          fileSink = AMRAudioFileSink::createNew(*env,
                                                 outFileName,
                                                 fileSinkBufferSize,
                                                 oneFilePerFrame);
...
void continueAfterPLAY(RTSPClient *, int resultCode, char *resultString) {
  ...
  if (duration > 0) {
    ...
    double rangeAdjustment = (session->playEndTime() - session->playStartTime())
        - (endTime - initialSeekTime);
    ...
    sessionTimerTask = env->taskScheduler().scheduleDelayedTask(uSecsToDelay,
                                                                (TaskFunc *) sessionTimerHandler,
                                                                (void *) NULL);
  }
      ...
      *env << "num_packets_lost\t"
      ...
      if (curQOSRecord != NULL) {
        ...
        *env << "measurement_sampling_interval_ms\t" << qosMeasurementIntervalMS
        ...
        if (curQOSRecord->kbits_per_second_max == 0) {
          ...
          *env << "kbits_per_second_ave\t"
               << (measurementTime == 0.0 ? 0.0 : 8 * curQOSRecord->kBytesTotal
                   / measurementTime) << "\n";
        ...
        *env << "packet_loss_percentage_max\t"
             << (packetLossFraction == 1.0 ? 100.0 : 100
                 * curQOSRecord->packet_loss_fraction_max) << "\n";
        ...
        if (stats != NULL) {
          ...
          *env << "inter_packet_gap_ms_ave\t"
               << (totNumPacketsReceived == 0 ? 0.0 : totalGapsMS
...
void continueAfterTEARDOWN(RTSPClient *,
                           int /*resultCode*/,
                           char *resultString) {
  delete[] resultString;
...
void signalHandlerShutdown(int /*sig*/) {
  *env << "Got shutdown signal\n";
  waitForResponseToTEARDOWN =
      False; // to ensure that we end, even if the server does not respond to our TEARDOWN
  shutdown(0);
}
