...
MPEG4ESVideoRTPSink
::MPEG4ESVideoRTPSink(UsageEnvironment &env,
                      Groupsock *RTPgs,
...
MPEG4ESVideoRTPSink::createNew(UsageEnvironment &env,
                               ...
                               u_int32_t rtpTimestampFrequency) {
  return new MPEG4ESVideoRTPSink(env,
                                 RTPgs,
                                 rtpPayloadFormat,
                                 rtpTimestampFrequency);
}
...
::doSpecialFrameHandling(unsigned fragmentationOffset,
  ...
  if (fragmentationOffset == 0) {
    ...
    u_int32_t startCode
        = (frameStart[0] << 24) | (frameStart[1] << 16) | (frameStart[2] << 8)
            | frameStart[3];
...
char const *MPEG4ESVideoRTPSink::auxSDPLine() {
  ...
  char const *fmtpFmt = "a=fmtp:%d "
      "profile-level-id=%d;"
      "config=";
  size_t fmtpFmtSize = strlen(fmtpFmt)
      + 3 /* max char len */
      + 3 /* max char len */
