...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
  ...
  do {
    ...
    if (fClientPortNum != 0
      ...
      if (isSSM()) {
        fRTPSocket =
            new Groupsock(env(), tempAddr, fSourceFilterAddr, fClientPortNum);
      } else {
      ...
      while (1) {
        ...
        if ((fClientPortNum & 1) != 0) { // it's odd
          ...
          Groupsock *existing =
              (Groupsock *) socketHashTable->Add((char const *) key,
                                                 fRTPSocket);
...
double MediaSubsession::getNormalPlayTime(struct timeval const &presentationTime) {
  ...
  if (!rtpSource()->hasBeenSynchronizedUsingRTCP()) {
    ...
    double nptOffset =
        (timestampOffset / (double) (rtpSource()->timestampFrequency()))
            * scale();
  ...
  do {
    ...
    if (strcmp(fProtocolName, "UDP") == 0) {
      ...
      if (strcmp(fCodecName, "QCELP") == 0) { // QCELP audio
        ...
        fReadSource =
            AMRAudioRTPSource::createNew(env(),
                                         fRTPSocket,
                                         fRTPSource,
                                         fRTPPayloadFormat,
                                         False /*isWideband*/,
        ...
        fReadSource = fRTPSource
            = TheoraVideoRTPSource::createNew(env(),
                                              fRTPSocket,
                                              fRTPPayloadFormat);
        ...
        Boolean
            expectDONFields = attrVal_unsigned("sprop-depack-buf-nalus") > 0;
        fReadSource = fRTPSource
...
SDPAttribute::SDPAttribute(char const *strValue, Boolean valueIsHexadecimal)
  ...
  if (fStrValue == NULL) {
    ...
    if (sscanf(fStrValueToLower, valueIsHexadecimal ? "%x" : "%d", &fIntValue)
        != 1) {
      fIntValue = 0;
    }
  }
